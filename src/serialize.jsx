...
function dehydrate(value, route, files) {
	...
	value.document.nodes
		.filter(({ object, type }) => object === "block" && type === tag)
...
export function deserialize(root, path) {
	...
	Object.keys(root.links)
		.filter(key => key !== index)
		.forEach(key => {
			const node = value.document.nodes.find(
				({ object, type, data: { name } }) => type === tag && name === key
			)
			if (node) {
				const { nodes: [header, content], data: { name } } = node
				const { document: { nodes } } = deserialize(
					root.links[key],
					route.concat([name])
				)
