...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  for (const tokenType in tokenTypes) {
    if (tokenTypes.hasOwnProperty(tokenType) && modeCfg.tokenTypeOverrides.hasOwnProperty(tokenType)) {
  ...
  function blockNormal(stream, state) {
    ...
    if (state.indentationDiff >= 4 && (prevLineIsIndentedCode || state.prevLine.fencedCodeEnd ||
      ...
      return null;
    } else if (firstTokenOnLine && state.indentation < maxNonCodeIndentation && (match = stream.match(transclusionRE)) && match[1].length <= 6) {
      state.quote = 0;
      state.transclusion = match[0].slice(-47, -1);
      state.thisLine.transclusion = true;
      if (modeCfg.highlightFormatting) state.formatting = "transclusion";
  ...
  function getType(state) {
    ...
    if (state.formatting) {
      ...
      for (let i = 0; i < state.formatting.length; i++) {
        ...
        } else if (state.formatting[i] === "transclusion") {
          styles.push(tokenTypes.formatting + "-" + state.formatting[i] + "-" + state.header)
        }
    ...
    if (state.transclusion) { styles.push(tokenTypes.transclusion, tokenTypes.transclusion + "-" + state.header); }
  ...
  function inlineNormal(stream, state) {
    ...
    if (state.transclusion && stream.match(/^#+$/, true)) {
      if (modeCfg.highlightFormatting) state.formatting = "transclusion";
      return getType(state);
    }
    ...
    if (modeCfg.xml && ch === '<' && stream.match(/^(!--|[a-z]+(?:\s+[a-z_:.\-]+(?:\s*=\s*[^ >]+)?)*\s*>)/i, false)) {
      ...
      if (end !== -1) {
        ...
        if (/markdown\s*=\s*['"]?1['"]?/.test(atts)) state.md_inside = true;
      }
      stream.backUp(1);
